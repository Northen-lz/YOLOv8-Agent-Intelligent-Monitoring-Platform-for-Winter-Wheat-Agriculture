...
class WheatVisionAgent(FunctionCallingAgent):
    ...
    def _vision_gate(self, image_path: str) -> str:
        """视觉兜底：问 Ollama qwen2.5vl「图中是否有小麦」。容错：任何失败 → unavailable"""
        try:
            result = self.vision_tool.describe(image_path, question=_VISION_GATE_QUESTION)
            if result.get("error"):
                return "unavailable"
            ...
            if "是" in text:
                return "wheat"
            return "unavailable"  # 回答不明确 → 降级给检测确认
        except Exception:  # noqa: BLE001
            return "unavailable"

    def recognize(self, image_path: str, conf: float = 0.5) -> dict:
        """识别门控（三级）：规则先筛 → 视觉兜底 → 检测确认。
        ...
        # 3) 规则疑似小麦 或 视觉不可用 → YOLOv8 检测确认（保留原语义）
        detection = self._detect(path, conf)
    ...
    def run(self, image_path=None, conf=0.5, *args, **kwargs):
        """识别门控 + 分析：
        ...
        try:
            gate = self.recognize(str(image_path), conf)
