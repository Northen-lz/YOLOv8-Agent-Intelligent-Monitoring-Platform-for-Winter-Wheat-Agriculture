...
class Config:
    ...
    # 检测流水记录文件（JSONL 逐条追加）。stats.json 只有累计值无时序，
    # 数据看板的「趋势图 + 最近检测流」读取本文件（每次检测一条：时间/株数/置信度/标注图）。
    DETECTION_LOG_FILE = os.getenv(
        "DETECTION_LOG_FILE",
        os.path.join(os.path.dirname(_PACKAGE_ROOT), "outputs", "detection_log.jsonl"),
    )
