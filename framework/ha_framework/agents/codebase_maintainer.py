...
实现跨会话的代码库维护任务管理：
1. 探索和理解代码库结构（TerminalTool 即时探索）
2. 记录发现的问题和改进点（NoteTool 结构化笔记）
3. 追踪长期的重构任务（NoteTool + ContextBuilder）
4. 在上下文窗口限制下保持连贯性（ContextBuilder GSSC 流水线）
"""
...
from ha_framework.context import ContextBuilder, ContextConfig, ContextPacket
from ha_framework.core.llm import HelloAgentsLLM
from ha_framework.core.message import Message
from ha_framework.tools import MemoryTool, NoteTool, TerminalTool
...
class CodebaseMaintainer:
    ...
    def __init__(
            ...
            notes_workspace: Optional[str] = None,
    ):
        ...
        # 初始化工具（notes_workspace 可选，默认 ./{project_name}_notes）
        self.memory_tool = MemoryTool(user_id=project_name)
        self.note_tool = NoteTool(workspace=notes_workspace or f"./{project_name}_notes")
    ...
    def _retrieve_relevant_notes(self, query: str, limit: int = 3) -> List[Dict]:
        ...
        try:
            ...
            # list 返回的索引元数据不含 content（blocker 优先检索走 list），
            # 逐个 read 补全，否则 _notes_to_packets 里 blocker 正文会丢失。
            for note_id, note in list(all_notes.items()):
                if note_id and not note.get('content'):
                    full = self.note_tool.run({
    ...
    def _notes_to_packets(self, notes: List[Dict]) -> List[ContextPacket]:
        ...
        packets = []
        for note in notes:
            # 根据笔记类型设置不同的相关性分数（文档 9.4.5: blocker > action > conclusion）
            relevance_map = {
                "blocker": 0.9,
                "action": 0.8,
                "task_state": 0.75,
...
""",
    ...
    def _postprocess_response(self, user_input: str, response: str):
        ...
        # 排除否定/无问题表述（如"无阻塞问题""未发现问题"），避免误建 blocker。
        response_lower = response.lower()
        issue_keywords = ["问题", "bug", "错误", "阻塞"]
        negations = [
            "无问题", "没问题", "未发现问题", "未发现", "不存在问题", "没有bug",
            "没有 bug", "无bug", "无 bug", "无错误", "无阻塞", "无严重问题",
            "良好", "正常", "健康",
        ]
        has_issue = any(kw in response_lower for kw in issue_keywords)
        has_negation = any(kw in response_lower for kw in negations)
        if has_issue and not has_negation:
            try:
                self.note_tool.run({
                    "action": "create",
