...
把 wheat/knowledge/ 下的文本知识灌入「语义记忆」双库：
...
通用能力来自 ha_framework；领域配置由 wheat.core.config.Config 提供。

用法（products/wheat 目录，需先启动 Qdrant + Neo4j）：
...
# 确保可导入 wheat 产品包；ha_framework 由 pip editable 安装可直接 import。
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from wheat.core.config import Config
from ha_framework.memory.base import MemoryConfig, MemoryItem
from ha_framework.memory.rag.document import _split_paragraphs_with_headings
from ha_framework.memory.types.semantic import SemanticMemory
